 ...
 * 4. Sends the PDF to client via WhatsApp (file attachment)
 * 5. Sends the PDF to client via Email (Gmail SMTP)
 * 6. Logs the invoice in Supabase (invoices table)
 *
 * Expected POST body:
 * {
...
/**
 * Send the invoice PDF to client via WhatsApp as a document attachment.
 * WhatsApp Cloud API requires the document to be hosted at a public URL.
 */
...
serve(async (req: Request) => {
  ...
  // ── Step 3: Upload PDF to Supabase Storage (needed for WhatsApp link) ───
  let pdfUrl = "";
  try {
  ...
  if (customer?.phone && pdfUrl) {
    try {
      const caption =
        `Invoice ${invoiceRef} - ${milestoneLabel}\n` +
        `Amount Due: ${formatCurrency(amount)}\n` +
        `Jabiru Ventures`;
      await sendWhatsAppDocument(customer.phone, pdfUrl, fileName, caption);
      console.log(`[Jabiru] ✅ Invoice sent via WhatsApp to ${customer.phone}`);
    } catch (err) {
      console.error("[Jabiru] ❌ WhatsApp send failed:", err);
    }
  } else {
    console.warn("[Jabiru] ⚠️ Skipped WhatsApp send (missing phone or PDF URL)");
  }
  ...
  // ── Step 6: Log invoice in Supabase ──────────────────────────────────────
  try {
